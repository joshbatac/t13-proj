<template>
  <div class="customer-order-view">
    ...
    <div v-if="!customerName">
      <h1>
        ...
        <div class="h1-sub">
          ...
          <br>
          <input class="input-num" type="text" id="phoneNumber" v-model="phoneNumber"
            pattern="[0-9]{3}-[0-9]{3}-[0-9]{4}" maxlength="10" placeholder="1234567890" @input="onInputChange"
            @keyup.enter="checkMember" required>
        </div>
      </h1>
    </div>
    <div v-else>
    ...
    <div class="content-container">
      ...
      <div class="basket">
        <Basket :items="basketItems" :customerID="this.customerID" :customerFName="this.customerFName"
          @removeItem="removeItemFromBasket" @fullRemove="fullRemove" @increaseCurrStorage="increaseCurrStorage"
          @updateInventory="updateInventory" />
      </div>
    </div>


    <customerInfoMod v-if="customerInfoModBool" :customerName="this.customerName" :customerFName="this.customerFName"
      :customerLName="this.customerLName" :customerID="this.customerID" :phoneNumber="this.phoneNumber"
      @boolChange="customerInfoModBoolChange" />

    <MemberSignUp v-if="memberSignUpBool" @boolChange="memberSignUpBoolChange" @finishedMSU="dataMSUtransfer" />

    <MemberSignUpConfirm v-if="showMSUC" :newCustomerFName="this.newCustomerFName"
      :newCustomerLName="this.newCustomerLName" :newCustomerPhoneNum="this.newCustomerPhoneNum" @cancel="toggleMSUC"
      @signUp="addCustomer" />

    <OrdersPopUp v-if="viewOrders" :customerID="this.customerID" @cancel="this.viewOrders = !this.viewOrders" />
    ...
    methods: {
        ...
        if (this.phoneNumber) { // Check if phoneNumber is not empty
          try { // Send a POST request to check the phone number
            const response = await axios.post(
              'https://arcane-headland-88481-37b7cd6b0bc8.herokuapp.com/check-phone-num', {
      ...
      async addCustomer() {
        try {
          ...
          const response = await axios.post(
            'https://arcane-headland-88481-37b7cd6b0bc8.herokuapp.com/customer-insert', {
