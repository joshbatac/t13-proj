...
<template>
    <div class="popup">
      ...
      <hr>
        <label for="removeItem">Select Item to Remove: </label>
        <select id="removeItem" v-model="selectedItem" class="custom-select">
          <option v-for="item in inventory" :key="item.ID" :value="item.ID">{{ item.name }}</option>
        </select>
        <label for="removeQuantity">
          Quantity to Remove: ( Current Storage: {{ selectedCurrentStock }}/{{ selectedMaxStock }} )
        </label>
        <input type="number" id="removeQuantity" v-model="removeQuantity" min="0" :max="selectedCurrentStock" />
        <hr><br>
        <button @click="removeStock" :disabled="removeQuantity <= 0 || removeQuantity > selectedCurrentStock" class="confirm-button">Remove Stock</button>
